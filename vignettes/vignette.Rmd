---
title: "Inferring stability of energy-flux food-webs with the fwstability R-package."
...
NOTE: The *fwstability* package is in the final stages of development and not yet available from CRAN. Please be aware of this when using the package in its current form.  
...
Daniëlle S.W. de Jonge, Peter C. de Ruiter, Johan van de Koppel, Dick van Oevelen. Inferring stability of energy-flux food-web models. In prep.  
...
# 1. Quick start
The *fwstability* package is not yet available from CRAN. If you wish to use the package as it currently is, you can download it from GitHub.  
...
First, you must prepare your energy-flux food-web model to be used by the package. A hypothetical example of what your data can look like:  
```{r eval=FALSE}
# 1. Biomasses for each food web compartment (abundance per unit area/volume).
...
frac_matrix["speciesA", "detritus"] <- flowmatrix["detritus", "speciesA"] * 
  (1-assimilation_eff["speciesA"]) / flowmatrix["speciesA", "detritus"]
frac_matrix["speciesB", "detritus"] <- flowmatrix["speciesA", "speciesB"] * 
  (1-assimilation_eff["speciesB"]) / flowmatrix["speciesB", "detritus"]
```
...
Now we can calculate a Jacobian matrix from the energy-flux model. The Jacobian matrix shows the per unit biomass effect of species i (in rows) on the population growth rate of species j (in columns) per unit time. In other words, the Jacobian matrix shows you the interaction strengths between species. The diagonal values are intraspecific effects, thus the density-dependent effects a compartment has on itself (works for both species and detritus). We use an example food web from the *fwmodels* package.   
```{r Jacobian_matrix}
model <- LovinkhoeveCP
...
JM4 <- getJacobian(model, diagonal = "model")

# The first 5 rows and columns of the Jacobian matrix:
print(JM4[1:5, 1:5])
```

Now we can infer stability from the Jacobian matrix. All methods are based on the concept that a Jacobian matrix is only stable if all real parts of the eigenvalues are negative. If the system is stable the system will asymptotically return to equilibrium after a small pulse perturbation. If the system is unstable the system will not return to equilibrium after a small pulse perturbation but move away from the equilibrium state. Beware that return rates and return times are NOT suitable measures to compare stability between different systems if these systems operate on different time-scales. If you want to compare relative stability between different systems, you should measure stability as the fraction of self-dampening necessary to stabilize the system.
```{r stability}
...
return_rate <- abs(s1) # per unit time
...
# simply set the diagonal to zero for now
JM_CP <- getJacobian(model = LovinkhoeveCP_noDet, diagonal = 0)
JM_IF <- getJacobian(model = LovinkhoeveIF_noDet, diagonal = 0)
```
...
# with a diagonal calculated from the model
...
The code takes a long time to execute, and is therefore not evaluated for building the vignette.

```{r upper_submarine_canyon, eval=FALSE}
# load the LIM library
library(LIM)

# Read-in and set up the linear inverse model
readLIMupper <- Read(system.file("extdata", "Nazare_Canyon_upper.input", package = "fwstability"))
...
The upper canyon has a stability of 'print(s)', thus an asymptotic rate of return of 'print(return_rate)'. The system with full self-dampening effects (upper limit) will return to equilibrium after a small pulse perturbation in 'print(return_time)' days, thus 'print(return_months)' months or 'print(return_years)' years.

```{r middle_submarine_canyon, eval=FALSE}
# load the LIM library
library(LIM)

# Read-in and set up the linear inverse model
readLIMmiddle <- Read(system.file("extdata", "Nazare_Canyon_middle.input", package = "fwstability"))
...
The middle canyon has a stability of 'print(s)', thus an asymptotic rate of return of 'print(return_rate)'. The system with full self-dampening effects (upper limit) will return to equilibrium after a small pulse perturbation in ' print(return_time)' days, thus 'print(return_months)' months or ' print(return_years)' years.

```{r lower_submarine_canyon, eval=FALSE}
# load the LIM library
library(LIM)

# Read-in and set up the linear inverse model
readLIMlower <- Read(system.file("extdata", "Nazare_Canyon_lower.input", package = "fwstability"))
...
The lower canyon has a stability of 'print(s)', thus an asymptotic rate of return of 'print(return_rate)'. The system with full self-dampening effects (upper limit) will return to equilibrium after a small pulse perturbation in ' print(return_time)' days, thus 'print(return_months)' months or ' print(return_years)' years.
...
```{r canyon_iterate_normalize, eval=FALSE}
...
```{r summary_stabilities, eval=FALSE}
...
```{r stability_characteristics, eval=FALSE}
