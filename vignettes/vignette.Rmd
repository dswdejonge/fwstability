...
# Quick start
This vignette explains how you can use the fwstability package to assess food web stability. There are basically five sections:  
1) how to prepare your food web model for stability analysis,  
2) how to derive the Jacobian matrix,
3) how to conduct stability analysis,  
4) how to assess your food web for characteristics related to stability, and  
5) examples with food webs from literature.  
...
In a linear inverse model (LIM) all processes in a food web are captured by a set of linear equations. These linear equations can be either equalities (the outcome of an equation should equal another value) or inequalities (the outcome of an equation should be equal or greater than another value). The equations can parameterized with known data, but often not all processes are studied well enough to quantify. The unknowns in the equations can then be found with linear programming, thereby fully quantifying your model. A simple example of a linear optimization problem can be found on \link[Wikipedia](https://en.wikipedia.org/wiki/Linear_programming#Example). More information on using LIM for food web quantification can be found in van Oevelen, D., van den Meersche, K., Meysman, F.J.R., Soetaert, K., Middelburg, J.J., Vézina, A.F., 2010. Quantifying food web flows using linear inverse models. Ecosystems 13, 32–45. https://doi.org/10.1007/s10021-009-9297-6.
...
- Dead compartments, defecation, and mortality can be omitted from your model if you don't want to include them. However, including assimilation and growth is required for all non-dead compartments. If you have no growth or assimilation data (for example for bacteria) you can assume the full ingestion is used for assimilation and/or growth and define your variable accordingly.
...
- a named vector with assimilation efficiencies for all non-dead compartments,  
- a named vector with growth efficiencies for all non-dead compartments.  
...
- a numeric vector with mortality rates (in the same unit as the flowmatrix). This information is obligator if you want to calculate diagonal values for the Jacobian matrix from the flux values.
