...
This vignette explains how you can use the *fwstability* package to assess stability of energy-flux food-web models. The subsections contain details about the features of the package and the scientific background. Reading this is highly recommended so that you can interpret your results correctly.   
...
Daniëlle S.W. de Jonge, Peter C. de Ruiter, Johan van de Koppel, Dick van Oevelen. Measuring stability of energy-flux food web models. In prep.
...
# build_vignettes = TRUE builds the vignette for you: this can 
# take some time as it includes some more complex calculations. 
# You can skip building the vignette by using build_vignettes = FALSE.
...
Now we can calculate a Jacobian matrix from the energy-flux model. The Jacobian matrix shows the per capita effect of species i (in rows) on the population growth rate of species j (in columns) per unit time. The diagonal values are intraspecific effects, thus the density-dependent effects a compartment has on itself (works for both species and detritus).  
```{r Jacobian_matrix}
model <- LovinkhoeveCP
